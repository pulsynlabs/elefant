...
## Process
1. Read the BLUEPRINT via `wf_blueprint.read({ workflowId })`.
2. Read the locked SPEC must-haves via `wf_spec.read({ workflowId, section: "must-haves" })` to keep them in context.
3. For each wave in order:
   a. Call `wf_state({ action: "update-wave", currentWave: N, totalWaves: M })` — emits `wave:started`.
   ...
   c. After each task completes, call `wf_chronicle.append({ kind: "task_completed", payload: { taskId, commitSha, outputs } })`.
...
4. After all tasks in a wave complete, call `wf_state({ action: "update-wave", currentWave: N+1 })` — emits `wave:completed`.
5. Save a checkpoint at each wave boundary via `wf_checkpoint.save({ context: { phase, currentWave, completedTasks } })`.
6. When all waves complete (currentWave === totalWaves), the workflow auto-progresses to the audit phase.

## Tools Used
- `wf_blueprint.read` — read the task plan
- `wf_spec.read` — keep must-haves in context
- `wf_state.update-wave` — advance wave counter
- `wf_chronicle.append` — log task events
- `spec_tasks.markComplete` / `spec_tasks.markBlocked` — update task status
- `wf_checkpoint.save` — save progress at wave boundaries
