...
## Prerequisites
- A checkpoint must exist for the workflow (`wf_checkpoint.list` returns at least one entry).
- The daemon must have access to the same project.

## Process
1. Call `wf_checkpoint.load({ workflowId })` to get the latest checkpoint.
2. Parse the checkpoint context: `phase`, `currentWave`, `totalWaves`, `completedTasks`, `pendingTasks`.
3. Restore the workflow phase via `wf_state({ action: "transition", to: checkpoint.phase, force: true })` — force-transition because normal phase validation doesn't account for restores.
4. Read the BLUEPRINT via `wf_blueprint.read({ workflowId })` to identify the next uncompleted task.
...
## Tools Used
- `wf_checkpoint.load` — load the saved checkpoint
- `wf_state.transition` — restore the phase (force)
- `wf_blueprint.read` — find the next task
- `wf_chronicle.read` — rebuild context
- `wf_adl.read` — recall decisions
...
**DON'T:** Assume the last checkpoint is always the right one — `wf_checkpoint.list` and let the user pick if multiple exist.
