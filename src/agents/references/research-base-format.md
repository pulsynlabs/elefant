---
id: research-base-format
title: Research Base Format
description: Frontmatter schema, section structure, and confidence levels for Elefant's markdown-db research base.
tags:
  - research-base
  - researcher
  - writer
...
Every Elefant project has a **Research Base** at `.elefant/markdown-db/` — a structured knowledge garden for long-form findings, comparisons, and reference notes. This is separate from the SQLite memory system (which holds ephemeral decision logs).

## Section Structure

The Research Base is organized into 8 section directories. Every file lives in exactly one section.
...
These are set by the system on `research_write` — do not specify manually unless you know the existing ID for an update:
...
| `workflow` | `string \| null` | `null` | Workflow slug for `research://` links; `null` for project-wide docs |
...
sources:
  ...
  - src/research/benchmarks/vector-search-results.json
confidence: high
...
Files in `99-scratch/` are exempt from strict frontmatter validation. They can have malformed or missing YAML blocks — the `research_read` tool uses lenient parsing for scratch files. Use this section for rough notes, works-in-progress, and temporary dumps.

### All Other Sections (00–06)

Files outside `99-scratch/` must pass full Zod validation. The `research_write` tool enforces this: if frontmatter fails validation, the write is rejected with a descriptive error.

## Cross-References with Memory

The Research Base stores long-form findings. The memory system stores ephemeral decision logs. Never write research findings directly to memory — use `research_write` instead.

When you need to reference a research finding in a memory entry, use the `research://` URI format (see `research-base-workflow` reference for citation details).

## Anti-Patterns

- **No Obsidian**: The Research Base is plain markdown. Do not emit `[[wikilinks]]` syntax. Use standard markdown links or `research://` URIs.
