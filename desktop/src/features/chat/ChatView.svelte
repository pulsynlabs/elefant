...
<div class="chat-view">
	...
	<div class="chat-input-area">
		<!--
			Branch navigator stacks ABOVE the redo banner. Branch
			context is the higher-level conversation-tree position
			("which branch am I on?"); redo context is the lower-level
			per-branch linear history. The navigator self-gates on
			`forkBranchCount > 0` and uses its own slide transition,
			so it cleanly collapses to zero height when no branches
			exist and never causes layout shift on the input below.
		-->
		<BranchNavigator />
