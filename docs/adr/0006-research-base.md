# ADR-0006: Research Base storage and embeddings
...
Elefant's Research Base needs a per-project, self-hosted storage and retrieval stack for long-form, citable markdown artifacts at `<projectRoot>/.elefant/markdown-db/`. The architecture-of-record in `.goopspec/research-base-system/SPEC.md` §1 requires a bundled vector index that works zero-config, preserves privacy by default, supports semantic and keyword search, and remains separate from the existing SQLite memory system.
...
Use SQLite plus the `sqlite-vec` extension as the default vector store, located at `<projectRoot>/.elefant/research-index.sqlite`. The default embedder is `Xenova/all-MiniLM-L6-v2` (384 dimensions) via `@xenova/transformers` / transformers.js, running on CPU by default and using WebGPU when available.
...
1. **LanceDB:** LanceDB has excellent developer experience and a good local-vector-store story, but its native build matrix does not cleanly cover every Elefant desktop target we need to support. Choosing it as the default would make distribution and support depend on a heavier native dependency surface than the Research Base needs.

2. **Qdrant:** Qdrant is a strong vector database with mature search features, but it requires running a separate server process. That violates the Research Base goal of a self-hosted, bundled, per-project knowledge store that works without operating another daemon beside Elefant.
...
5. **pgvector:** pgvector is excellent when Postgres is already part of the deployment, but requiring Postgres for a per-project Research Base is too heavy. Elefant needs a single-file local index that can be created lazily beside project research files.
...
Risk R7 is accepted with an explicit provider-switch policy: each chunk records its embedding dimension, and switching to a provider with a different dimension triggers a forced non-destructive reindex. The reindex rewrites chunks and embeddings only, never source markdown files, and emits a `research:provider-changed` WebSocket event so the UI can request user confirmation.

### Files modified
- `docs/adr/0006-research-base.md` — Recorded the Research Base storage and embedding-provider architecture decision, rejected alternatives, R1 fallback, R7 dimension-mismatch reindex policy, and Obsidian out-of-scope boundary
...
### Positive
- The default stack is local-first, per-project, and zero-config while preserving a single-file index at `.elefant/research-index.sqlite`
...
This ADR introduces no code-level compatibility break. The Research Base is new, lives under `.elefant/markdown-db/` and `.elefant/research-index.sqlite`, and remains separate from the existing SQLite memory system. Provider changes are non-destructive by policy: markdown source files are preserved and only derived chunks/index rows are rebuilt.

## Evidence

- `.goopspec/research-base-system/SPEC.md` §1 — Locks the architecture-of-record: `.elefant/markdown-db/`, SQLite + `sqlite-vec`, `Xenova/all-MiniLM-L6-v2`, provider matrix, HNSW fallback, `research:provider-changed`, and ADR-0006 scope
- `.goopspec/research-base-system/SPEC.md` §2.2 — Defines MR-6 through MR-10 for the bundled vector index, pluggable providers, hardware recommendation, non-destructive provider switching, and keyword fallback
- `.goopspec/research-base-system/SPEC.md` §4 — Marks native Obsidian integration / vault adapter / plugin as out of scope
- `.goopspec/research-base-system/SPEC.md` §6 — Records R1 (`sqlite-vec` native build coverage) and R7 (mixed-dimension provider switching) mitigations
- `.goopspec/research-base-system/SPEC.md` §9 — Maps MR-40 to ADR-0006 and `docs/research-base.md`

## Follow-ups

W1.2 validates `sqlite-vec` loading under Bun on Linux and confirms whether the default path is viable on the primary CI target. W1.3 validates `@xenova/transformers` under Bun and measures MiniLM CPU latency. Later provider-switch implementation must enforce the dimension-mismatch reindex policy and emit `research:provider-changed` before rebuilding derived chunks.

---

*Captured during Wave 1 of the research-base-system workflow.*
